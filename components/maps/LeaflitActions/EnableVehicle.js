...
export default function DisableVehicle({ show, setShow , selectedVehicles }) {
  const {
    auth: { user },
    config: { darkMode },
  ...
  const handleRq = async (e) => {
    ...
    setloading(true);
     const SerialNumber = document
      .getElementById("EnableVehicleBtn")
      .getAttribute("data-id");
    const vehicle = selectedVehicles.filter(veh => +veh.SerialNumber === +SerialNumber)[0]   

    await axios
      .post("https://sharex-api-hcr64pytia-uc.a.run.app/vehicle/v/lock", {
      ...
      .catch((er) => {
        ...
        toast.error("Error For Vehicle Enabled.");
      })
      .finally(() => {
        setloading(false);
        handleClose();
      ...
      >
        <Modal.Header closeButton className={`${Dark}`}>
          <Modal.Title id="contained-modal-title-vcenter">
            <p className="lead">
              {t("are_you_sure_you_want_to_enable_this_vehicle?_key")}
            </p>
          </Modal.Title>
        </Modal.Header>
