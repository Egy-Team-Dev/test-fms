...
const ShowGeofence = ({
  ...
  const handleShowFence = (params) => {
    ...
    if (show.length === 0) {
      if (
        path?.some((fenc) => fenc != null) &&
        !path?.some((fenc) => isNaN(fenc)) &&
        path?.some((fenc) => fenc != undefined) &&
        path?.some((fenc) => fenc != "undefined")
      ) {
        if (params.data.GeoFenceType === 1) {
          // polygon
          return L.polygon(path, options).addTo(drawGroup);
        }
        // check if a circle
        else if (
          params.data.GeoFenceType === 2 &&
          params.data?.GeofenceRadius != null
        ) {
          return L.circle(path, {
            ...options,
            radius: +params.data.GeofenceRadius,
          }).addTo(drawGroup);
        } else if (params.data.GeoFenceType === 3) {
          // rectangle
          return L.rectangle(path, options).addTo(drawGroup);
        }
      }
    } else {
