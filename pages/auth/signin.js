...
const Signin = () => {
  ...
  const { nw, at } = router.query;
  const session = useSession();
  const userRole = session[0]?.user?.mongoRole;
  ...
  // useEffect(()=>{
  //   darkMode = (typeof window !== undefined || !localStorage.getItem("lastConfig"))?
  //   JSON.parse(localStorage.getItem("lastConfig")).darkMode: false
  ...
  useEffect(() => {
    if (nw !== undefined) {
      ...
      setTimeout(() => {
        ...
        //     encryptName("userData", JSON.stringify({ darkMode: darkMode }))
        //   );
        // }
    ...
    //     dispatch(
    //       setSystemConfig({
    //         logo: res?.data?.setting[0]?.icon,
    //         title: res?.data?.setting[0]?.title,
    //       })
    //     );
    //     setLogoUrl(res.data.setting[0].icon);
    //     setTitle(res.data.setting[0].title);
    //   };
